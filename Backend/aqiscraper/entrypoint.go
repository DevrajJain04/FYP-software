...
func startBackgroundFetcher() {
	...
	log.Println("🔄 Starting initial station data fetch...")
...
func main() {
	...
	log.Printf("📊 Configuration: fetch_interval=%v station_ttl=%v h3_resolution=%d",
		config.FetchInterval, config.StationTTL, config.H3Resolution)
	...
	http.HandleFunc("/h3/batch", corsMiddleware(batchH3Handler)) // Batch H3 lookups for performance
	...
	log.Printf("📡 Endpoints: /health, /stations, /stats, /h3, /h3/batch, /nearest, /radius, /fetch")
